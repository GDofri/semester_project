...
from src.models.dataset import split_data_into_datasets
from src.models.dataset import collate_fn_wo_mask
...
def train(
    ...
    train_loader = DataLoader(train_dataset, batch_size=batch_size, shuffle=True, collate_fn=collate_fn_wo_mask)
    val_loader = DataLoader(val_dataset, batch_size=batch_size, shuffle=False, collate_fn=collate_fn_wo_mask)
    test_loader = DataLoader(test_dataset, batch_size=batch_size, shuffle=False, collate_fn=collate_fn_wo_mask)
    print("Data loaders initialized")

    # Initialize the model, optimizer, and loss function
    if model is None:
        model = SimpleCNNWithNumerics()
    ...
    for epoch in range(num_epochs):
        ...
        for padded_sequences, numerics, targets in train_loader_tqdm:
            padded_sequences = padded_sequences.to(device)
            numerics = numerics.to(device)
            targets = targets.to(device)

            optimizer.zero_grad()
            outputs = model(padded_sequences, numerics).squeeze(-1)
        ...
        with torch.no_grad():
            for padded_sequences, numerics, targets in val_loader_tqdm:
                padded_sequences = padded_sequences.to(device)
                numerics = numerics.to(device)
                targets = targets.to(device)
                outputs = model(padded_sequences, numerics).squeeze(-1)
    ...
    with torch.no_grad():
        for padded_sequences, numerics, targets in test_loader_tqdm:
            padded_sequences = padded_sequences.to(device)
            numerics = numerics.to(device)
            targets = targets.to(device)
